...
const projectDetailCopyByLocale: Record<SupportedLocale, ProjectDetailCopy> = {
  "zh-CN": {
    notFoundTitle: "项目不存在 | Personal Showcase",
    backToWork: "← 返回项目列表",
    website: "网站",
    visitWebsite: "访问网站 ↗",
    role: "角色",
  ...
  en: {
    notFoundTitle: "Project Not Found | Personal Showcase",
...
export async function buildProjectMetadata(slug: string, locale: SupportedLocale): Promise<Metadata> {
  const project = getProjectBySlug(slug);
  const copy = projectDetailCopyByLocale[locale];
  ...
  return {
    title: `${localizedProject.title} | Personal Showcase`,
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildProjectMetadata(slug, defaultLocale);
}
...
export async function ProjectDetailPageView({ slug, locale = defaultLocale }: ProjectDetailViewProps) {
  const project = getProjectBySlug(slug);
  const copy = projectDetailCopyByLocale[locale];
          ...
          <article className="mt-6 rounded-3xl border border-zinc-200 bg-white/85 p-6 shadow-[0_24px_70px_-42px_rgba(24,24,27,0.55)] sm:p-10 dark:border-zinc-800 dark:bg-zinc-900/75 dark:shadow-[0_30px_80px_-52px_rgba(0,0,0,0.85)]">
            <div className="relative overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-900 dark:border-zinc-700">
              <div className="relative aspect-[16/8]">
                <Image
                  src={localizedProject.coverImage}
                  alt={localizedProject.title}
                  fill
                  priority
                  sizes="(min-width: 1024px) 960px, 100vw"
                  className={`h-full w-full opacity-90 ${
                    localizedProject.imageFit === "contain" ? "object-contain bg-zinc-950 p-2" : "object-cover"
                  }`}
                />
              </div>
              ...
              <div className="absolute left-4 top-4 inline-flex rounded-full border border-white/35 bg-black/20 px-2.5 py-1 text-[11px] tracking-[0.08em] uppercase text-white">
                {getProjectStatusLabel(localizedProject.status, locale)}
              </div>
              <div className="absolute bottom-4 left-4 right-4 flex flex-wrap items-end justify-between gap-3">
                <div>
                  <p className="text-xs tracking-[0.1em] uppercase text-zinc-100/80">{localizedProject.category}</p>
                  <h1 className="font-display mt-1 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
                    {localizedProject.title}
                  </h1>
                </div>
                <span className="text-xs tracking-[0.1em] uppercase text-zinc-100/80">{localizedProject.timeline}</span>
              </div>
            </div>

            <p className="mt-8 max-w-[68ch] text-base leading-8 text-zinc-600 dark:text-zinc-300">{localizedProject.overview}</p>
            {localizedProject.websiteUrl ? (
              <section className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-50/80 p-5 dark:border-zinc-800 dark:bg-zinc-900/60">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.website}</h2>
                    <p className="mt-2 break-all text-sm leading-7 text-zinc-700 dark:text-zinc-300">{localizedProject.websiteUrl}</p>
                  </div>
                  <a
                    href={localizedProject.websiteUrl}
                    target="_blank"
                    rel="noreferrer"
            ...
            <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-12">
              <section className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-5 md:col-span-4 dark:border-zinc-800 dark:bg-zinc-900/60">
                <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.role}</h2>
                <p className="mt-3 text-sm leading-7 text-zinc-700 dark:text-zinc-300">{localizedProject.role}</p>
              </section>
              <section className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-5 md:col-span-8 dark:border-zinc-800 dark:bg-zinc-900/60">
                <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.challenge}</h2>
                <p className="mt-3 text-sm leading-7 text-zinc-700 dark:text-zinc-300">{localizedProject.challenge}</p>
              </section>
            </div>

            <section className="mt-8 border-t border-zinc-200 pt-6 dark:border-zinc-800">
              <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.solution}</h2>
              <ul className="mt-3 space-y-2 text-sm leading-7 text-zinc-700 dark:text-zinc-300">
                {localizedProject.solution.map((item) => (
                  <li key={item}>• {item}</li>
                ))}
              </ul>
            </section>
            ...
            <section className="mt-8 border-t border-zinc-200 pt-6 dark:border-zinc-800">
              <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.outcome}</h2>
              <ul className="mt-3 space-y-2 text-sm leading-7 text-zinc-700 dark:text-zinc-300">
                {localizedProject.impact.map((item) => (
                  <li key={item}>• {item}</li>
                ))}
                {localizedProject.outcomes.map((item) => (
                  <li key={item}>• {item}</li>
                ))}
              </ul>
            </section>
            ...
            <section className="mt-8 border-t border-zinc-200 pt-6 dark:border-zinc-800">
              <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.stack}</h2>
...
export default async function ProjectDetailPage({ params }: PageProps) {
  const { slug } = await params;
  return <ProjectDetailPageView slug={slug} locale={defaultLocale} />;
}
