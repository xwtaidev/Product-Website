...
export default function BlogPageContent({ blogPosts, locale = defaultLocale }: BlogPageContentProps) {
  ...
  const blogIndexHref = withLocalePath(locale, "/blog");
  ...
  const copy =
    locale === "en"
      ? {
          ...
          all: "All",
          deepDive: "Need Deep Dive",
          deepDiveDescription:
            "If you want the full implementation details and architecture background, I can share this article together with related cases so you can quickly evaluate reuse in your AI system.",
          browseWork: "Browse work",
        }
      : {
          featuredWriting: "精选文章",
          readArticle: "阅读文章",
          emptyCategory: "当前分类下暂无文章。",
          categories: "分类",
          all: "全部",
          deepDive: "需要更深入拆解？",
          deepDiveDescription:
      ...
      <div className="space-y-6">
        {featuredPost ? (
          <article className="group overflow-hidden rounded-3xl border border-zinc-200 bg-white/82 fade-up dark:border-zinc-800 dark:bg-zinc-900/72">
            <Link href={withLocalePath(locale, `/blog/${featuredPost.slug}`)} className="block">
        ...
        <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white/70 divide-y divide-zinc-200/80 fade-up dark:border-zinc-800 dark:bg-zinc-900/58 dark:divide-zinc-800/90">
          {otherPosts.map((post) => (
            <article key={post.slug} className="group p-4 sm:p-5">
              <Link href={withLocalePath(locale, `/blog/${post.slug}`)} className="grid grid-cols-1 gap-4 sm:grid-cols-[136px_1fr] sm:items-start">
      ...
      <aside className="fade-up">
        <div className="lg:sticky lg:top-24 space-y-4">
          <section className="rounded-2xl border border-zinc-200 bg-white/82 p-5 dark:border-zinc-800 dark:bg-zinc-900/72">
            <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.categories}</h2>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {categories.map((category) => (
                <Link
                  key={category}
                  href={`${blogIndexHref}?category=${encodeURIComponent(category)}`}
                  className={`rounded-full border px-2.5 py-1 text-[11px] transition ${
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white/82 p-5 dark:border-zinc-800 dark:bg-zinc-900/72">
            <h2 className="font-display text-xs tracking-[0.12em] uppercase text-zinc-500 dark:text-zinc-400">{copy.deepDive}</h2>
            <p className="mt-3 text-sm leading-7 text-zinc-600 dark:text-zinc-300">
              {copy.deepDiveDescription}
            </p>
